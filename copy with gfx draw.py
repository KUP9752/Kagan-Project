...
class EnemyCone(pygame.sprite.Sprite):
    def __init__(self, x, y, start_angle, end_angle):
        super().__init__()
        self.width = 50
    ...
    def draw(self):
        pygame.gfxdraw.pie(self.image, self.rect.x, self.rect.y, 50, self.start_angle, self.end_angle, self.colour)
...
def create_enemy(x, y, e_type, facing):
    ...
    if e_type == 1:
        ...
        create_visioncone((x + (enemy.width // 2)), (y + (enemy.height // 2)), facing)
    elif e_type == 2:
...
def create_visioncone(x, y, facing):
    if facing == 61:
        visioncone = EnemyCone(x, y, 235, 305)
        cone_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 62:
        visioncone = EnemyCone(x, y, 55, 125)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 63:
        visioncone = EnemyCone(x, y, 325, 35)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 64:
        visioncone = EnemyCone(x, y, 145, 215)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
