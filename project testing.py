...
class EnemyCone(pygame.sprite.Sprite):
    def __init__(self, x, y, image, w, h,index):
        super().__init__()


        self.width = w
        self.height = h
        self.graphic = ConeGraphics
        self.colour = GREY
        self.graphic_index = index

        self.image = pygame.transform.scale(self.graphic[self.graphic_index], (self.width, self.height))
...
def create_visioncone(x, y, facing):    #for U and D, w= 82, h = 100; For R and L w = 100, h =82
    if facing == 61:
        visioncone = EnemyCone(x, y, ConeU, 82,100,0)
        #cone_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 62:
        visioncone = EnemyCone(x, y, ConeD,82,100,1)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 63:
        visioncone = EnemyCone(x, y, ConeR, 100, 82,2)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
    if facing == 64:
        visioncone = EnemyCone(x, y, ConeL, 100, 82,3)
        enemy_group.add(visioncone)
        all_sprites_group.add(visioncone)
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if play_game and not (
        pause_menu) and not end_level and event.type == pygame.MOUSEBUTTONDOWN:  # checks if a level is clicked
            for counter in range(0, 10):
    ...
    if not level_running and not pause_menu:  # -whether a level is running
        if not play_game and not end_level:  # -nothing running displays title screen
